...
void skyDome::calcCloudDomeCoords(void)
{
	...
	for(int x=0; x<=8; x++)
	{
		...
		for(int y=0; y<=8; y++)
		{
			...
			cloudDomeCoords[x][y].x = (x - 4) * 100;
			cloudDomeCoords[x][y].y = sqrt(zSq);
			cloudDomeCoords[x][y].z = (y - 4) * 100;
...
void skyDome::calcSkyDomeCoords(void)
{
	...
	for(int i=0; i<=heightSections; i++)
	{
		...
		{
			sphereCoords[i][j].x = tempRadius * cos(j * radiusStep);
			sphereCoords[i][j].y = tempRadius * sin(j * radiusStep);
			sphereCoords[i][j].z = tempZ;
		}
	}	
}
