...
=============
*/
unsigned char *getRGBA2 (FILE *s, int size)
{
    unsigned char *rgba;
    unsigned char temp;
    int bread;
...
=============
*/
unsigned char *getRGB2 (FILE *s, int size)
{
    unsigned char *rgb;
    unsigned char temp;
    int bread;
...
=============
*/
unsigned char *getGray2 (FILE *s, int size)
{
...
=============
*/
char *getData2 (FILE *s, int sz, int iBits)
{
    if (iBits == 32)
        return (char *)getRGBA2 (s, sz);
    else if (iBits == 24)
        return (char *)getRGB2 (s, sz);	
    else if (iBits == 8)
        return (char *)getGray2 (s, sz);
}
/*
=============
returnError
...
*/
int returnError2 (FILE *s, int error)
{
    fclose (s);
    return error;
}
...
int loadTGA (char *name, int id)
{
    ...
    if (type[1] != 0 || (type[2] != 2 && type[2] != 3))
        returnError2 (s, TGA_BAD_IMAGE_TYPE);
    ...
    if (!checkSize2 (imageWidth) || !checkSize2 (imageHeight))
        returnError2 (s, TGA_BAD_DIMENSION);

    /* make sure we are loading a supported type */
    if (imageBits != 32 && imageBits != 24 && imageBits != 8)
        returnError2 (s, TGA_BAD_BITS);

    imageData = (unsigned char *)getData2 (s, size, imageBits);

    /* no image data */
    if (imageData == NULL)
        returnError2 (s, TGA_BAD_DATA);
