#include "rctgl-Wall.h"

using namespace RCTWall;  //import path constants
using namespace RCTTextureManager;

//#define TEST_OPTIMIZATION

RCTGLWallSystem::RCTGLWallSystem()
{

}


bool RCTGLWallSystem::loadOffset(uchar *data, uchar x, uchar z)
{	
	RCTGLWallElement tmpWall;

	tmpWall.baseHeight = 0;
	tmpWall.itemHeight = 0;
	tmpWall.descriptor = 0;

	tmpWall.descriptor |= tileType;
	...
	{
	case 0x00:
		tmpWall.descriptor |= SW_QUAD;
		break;
	case 0x01:
		tmpWall.descriptor |= NW_QUAD;
		break;
	case 0x02:
		tmpWall.descriptor |= NE_QUAD;
		break;
	case 0x03:
		tmpWall.descriptor |= SE_QUAD;
		break;
	}	
	...
	{
	case 0x00:
		tmpWall.descriptor |= FACES_WEST;
		break;
	case 0x01:
		tmpWall.descriptor |= FACES_NORTH;
		break;
	case 0x02:
		tmpWall.descriptor |= FACES_EAST;
		break;
	case 0x03:
		tmpWall.descriptor |= FACES_SOUTH;
		break;
	}
	
	
	//base height = byte 2
	tmpWall.baseHeight = data[2] / 4;

	//item height = byte 3
	tmpWall.itemHeight = data[3] / 4;
	
	//sometimes this value is 0
	//anyone know why?
	if(tmpWall.itemHeight)
		tmpWall.itemHeight -= tmpWall.baseHeight;

	//item id = byte 4
	tmpWall.WallID = data[4];

	//item index = upper 6 bits of byte 5
	tmpWall.index = (data[4] >> 2);	

	m_wallItems[x][z].push_back(tmpWall);

	return true;
}

void RCTGLWallSystem::compile()
{
	DebugLog::beginTask("RCTGLWallSystem::compile");
	...
	for(i=0; i<128; i++)
	{
		for(j=0; j<128; j++)
		{
			for(k=0; k<m_scenItems[i][j].size(); k++)
			{
				...
				{
					switch(m_scenItems[i][j][k].WallType)
					{
					case Wall_TREE:
					{
						//build a tree
						...
						tmp.setBaseRGB(rgb);
						tmp.setTextureID(m_treeTextures[m_scenItems[i][j][k].WallID]);
...
void RCTGLWallSystem::draw(uchar minX, uchar minZ, uchar maxX, uchar maxZ) const
{	
	uchar i, j, k, l;
		...
		for(j=minZ; j<maxZ; j++)
		{
			for(k=0; k<m_scenItems[i][j].size(); k++)
			{
				if(theFrustum.isCubeInFrustum((float)(i*UNITWIDTH),
					...
					(float)(UNITWIDTH)))	
				{
					for(l=0; l<m_wallItems[i][j][k].surfaces.size(); l++)
					{	
						m_wallItems[i][j][k].surfaces[l].draw();
					}
				}				
			}
		}
...
void RCTGLWallSystem::clear()
{
	DebugLog::beginTask("RCTGLWallSystem::clear");
...
void RCTGLWallSystem::loadTreeTextures()
{
	RCTGLTextureManager texMan;

	DebugLog::beginTask("RCTGLWallSystem::loadTreeTextures");
...
void RCTGLWallSystem::loadFlowerTextures()
{
	RCTGLTextureManager texMan;

	DebugLog::beginTask("RCTGLWallSystem::loadFlowerTextures");
	DebugLog::endTask();
}

void RCTGLWallSystem::loadTextures()
{
	DebugLog::beginTask("RCTGLWallSystem::loadTextures");
...
bool RCTGLWallSystem::isItemTree(uchar type)
{
